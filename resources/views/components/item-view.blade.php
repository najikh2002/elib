...
@section('content')
    ...
    <div class="py-12 px-3 md:px-0">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 flex flex-col-reverse gap-6 md:gap-0 md:flex-row">
            {{-- Desc --}}
            <div class="basis-3/4 flex flex-col gap-3 md:gap-[100px] md:pr-12">
                {{-- Title --}}
                <div class="flex flex-col md:gap-[50px]">
                    <h3 class="text-[33px] md:text-[48px] font-light font-serif">{{ $buku->judulbuku }}</h3>
                    <p>Oleh
            ...
            <div class="flex flex-col basis-1/4 justify-center items-center shadow-md rounded-md h-fit pb-12 pt-3 gap-3 bg-slate-100">
                @if ($buku->sampulbuku)
                        <div class="p-2 bg-slate-100 mb-[100px] md:mb-6 w-[95%] h-[450px]">
                            <div class="relative w-full h-80">
                                <img src="{{ asset("storage/$buku->sampulbuku") }}" alt="Sampul Buku" class="object-cover w-full">
                            </div>
                        </div>
                        @else
                        <div class="p-2 bg-slate-200 mb-[100px] md:mb-6 w-[95%]">
                            <div class="relative w-full h-80">
                ...
                @if ($pinjam)
                    <a href="/detail/{{ $buku->kodebuku }}/read" class="py-2 w-[200px] rounded-md text-center bg-black uppercase text-white hover:opacity-80 transition-all duration-200">Baca</a>
                    <form action="/user/hapus-sesi/{{ $buku->kodebuku }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="py-2 w-[200px] rounded-md bg-black uppercase text-white hover:opacity-80 transition-all duration-200 text-center">Kembalikan</button>
                @else
